...
impl Data {
    ...
    pub(super) fn top(&mut self, ui: &mut Ui) {
        ui.heading(ui.localize("loaded-files")).on_hover_ui(|ui| {
            ui.label(ui.localize("loaded-files.hover"));
        });
        ui.separator();
        // Toggle
        if ui
            ...
            .on_hover_ui(|ui| {
                ui.label(ui.localize("toggle-all"));
            })
            .on_hover_ui(|ui| {
                ui.label(ui.localize("toggle-all.hover"));
            })
            .clicked()
        {
